...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // Quick credit check with timeout - optimized for development
        let remainingCredits = 50;
        if (process.env.NODE_ENV !== 'development') {
            const creditCheckPromise = getRemainingCredits({
            ...
            remainingCredits = await Promise.race([
                ...
                new Promise<number>((resolve) => {
                    setTimeout(() => resolve(50), 1000); // Fallback to 50 credits after 1 second
                })
            ]);
        }
...
function createCompletionStream({
    ...
    return new ReadableStream({
        async start(controller) {
            ...
            // Send immediate status message to make UI feel responsive
