...
export const ChatInput = ({
    ...
    const { editor } = useChatEditor({
        ...
        onInit: ({ editor }) => {
            if (typeof window !== 'undefined' && !isFollowUp && !isSignedIn) {
                // Don't restore draft messages on localhost
                ...
                if (!isLocalhost) {
                    ...
                    // Clear any existing draft message on localhost
        ...
        onUpdate: ({ editor }) => {
            if (typeof window !== 'undefined' && !isFollowUp) {
                // Don't save draft messages on localhost
                const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
                if (!isLocalhost) {
    ...
    const sendMessage = async () => {
        ...
        const threadItemsPromise = currentThreadId ? getThreadItems(currentThreadId.toString()) : Promise.resolve([]);
        
        window.localStorage.removeItem('draft-message');
        editor.commands.clearContent();
        clearImageAttachment();

        threadItemsPromise.then(threadItems => {
        ...
        >
            <AnimatePresence mode="wait">
                <motion.h1
                    ...
                    className="from-muted-foreground/50 via-muted-foreground/40 to-muted-foreground/20 bg-gradient-to-r bg-clip-text text-center text-[32px] font-semibold tracking-tight text-transparent flex items-center gap-2"
                >
                    {greeting}
                    {isLoaded && user && (
                        <>
